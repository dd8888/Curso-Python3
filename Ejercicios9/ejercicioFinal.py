import csv
def LeerPartidos():
	"""Funcion que lee el fichero CSV y devuelve los datos del mismo en una 
...
def Equipos(liga):
	"""Funcion que recibe la lista de diccionarios con los datos de la liga y devuelve un
	 conjunto con los equipos de la liga."""
	return(tuple(set([partido["equipo1"] for partido in liga])))


def QuienGana(resultado):
	"""Funcion que recibe un resultado y devuelve un 0 si es un empate, un 1 si
...
def Puntos(info):
	"""* `Puntos(info)`: Funcion que recibe una lista con los partidos ganados,
	 empatados y perdidos y devuelve los puntos obtenidos."""
	return 3*info[0]+info[2]

def InfoEquipos(liga,*equipos):
	"""Funcion que recibe la lista de diccionarios con los datos de la liga y el conjunto de
...
def Clasificacion(datos):
	"""Recibe la lista generada con la funcion anterior y la ordena segun 
	el numero de puntos."""
...
def impClasificacion(liga):
	"""recibe la lista de diccionarios con losnerado a parir del fichero csv, genera los datos
	 de la clasificacion y los imprime por pantalla"""
